...
impl ColorMergeWidget {
    ...
    fn inputs_for_mode(mode: ColorMode) -> Vec<PortDef> {
...
impl NodeWidget for ColorMergeWidget {
    ...
    fn ui_inputs(&self) -> Vec<UiPortDef> {
        Self::inputs_for_mode(self.mode).iter().map(UiPortDef::from_def).collect()
    }
    ...
    fn show_content(&mut self, ui: &mut Ui, _zoom: f32) {
        ...
        // Update cached mode for port rendering.
        if mode != self.mode {
            self.mode = mode;
        }
...
impl ColorSplitWidget {
    ...
    fn outputs_for_mode(mode: ColorMode) -> Vec<PortDef> {
        mode.channel_names().iter().map(|n| PortDef::new(*n, PortType::Untyped)).collect()
    }
}
...
impl NodeWidget for ColorSplitWidget {
    ...
    fn ui_outputs(&self) -> Vec<UiPortDef> {
        Self::outputs_for_mode(self.mode).iter().map(UiPortDef::from_def).collect()
    }
